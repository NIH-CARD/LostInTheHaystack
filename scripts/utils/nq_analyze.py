    ...
    for model, gdf in all_results.groupby("model", sort=False):
        stats = {
            s: compute_fn(pd.Series(np.concatenate([
                gdf[f"{s}@{d}_{metric}"].dropna().to_numpy() for d in depths if f"{s}@{d}_{metric}" in gdf
            ]))) for s in size_map
        }
...
def run_model_analysis(model_config: dict, bench_config: dict) -> None:
    ...
    print_(f"{bench_name} analysis on {model_id} complete → {output_dir}", fun="{+}")
...
def run_benchmark_analysis(bench_config: dict) -> None:
    ...
    plot_heatmap(baseline_df, list(size_map), bench_name, output_dir, metric_label, vmin=0.0, vmax=1.0, baseline=True)
